...
function Request_API(category){//Request_API("DL");
...
function Create_Question(){//생성하기를 누르면 입력된 정보 다 긁어오고 기존 정보 초기화
    ...
    Photo_Obj.q = Post_Img(document.getElementById("Question_file").value);//질문내용 사진값
    ...
    switch(type){
        case "2":
            for(let i=1;i<= 2;i++){
                ...
                if(AnswerImg.children.length > 0){//사진값이 있는지 확인
                    if(i == 1){
                        Photo_Obj.a1 = Post_Img(AnswerImg.children[0].src);//질문내용 이미지 링크값 가져오기
                    } else if(i == 2){
                        Photo_Obj.a2 = Post_Img(AnswerImg.children[0].src);
                    }
                }
            }
            break;
        case "3":
            for(let i=1;i<= 3;i++){
                ...
                if(AnswerImg.children.length > 0){//사진값이 있는지 확인
                    if(i == 1){
                        Photo_Obj.a1 = Post_Img(AnswerImg.children[0].src);
                    } else if(i == 2){
                        Photo_Obj.a2 = Post_Img(AnswerImg.children[0].src);
                    } else if(i == 3){
                        Photo_Obj.a3 = Post_Img(AnswerImg.children[0].src);
                    }
                }
            }
            break;
            ...
            for(let i=1;i<= 4;i++){
                ...
                if(AnswerImg.children.length > 0){//사진값이 있는지 확인
                    if(i == 1){
                        ...
                        Photo_Obj.a4 = Post_Img(AnswerImg.children[0].src);
                    }
                }
            }
            break;
...
function Request_API_Question(data){//새로운 질문 Post
    // 질문 생성: post
    // http://ec2-43-201-19-40.ap-northeast-2.compute.amazonaws.com/api/create_question
    // "category" : "DL",
    // "type" : "2",
